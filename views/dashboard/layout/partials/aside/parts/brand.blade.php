    ...
    <div class="kt-aside__brand-logo">
        <a href="{{route(userType()::prefix().'dashboard')}}">
            @if(Auth::user()->type==userType()::admin)
                <img style="width: 90%" alt="Logo" src="{{ asset('images/elbadry.png') }}" />
            @elseif(Auth::user()->type==userType()::vendor)
                <?php
                $data=Auth::user()->metas()->where(['key'=>"logo"])->first();
                $image=$data?$data->value: 'images/elbadry.png' ;
                ?>
                <img style="width: 90%" alt="Logo" src="{{ShowImage($image)}}" />
            @endif
        </a>
    </div>
